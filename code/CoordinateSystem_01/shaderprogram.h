...
public:
    ...
    void setMat4(const char *name, const QMatrix4x4& mat, bool useShader = false)
    {
        if (useShader)
            use();

        m_funcs->glUniformMatrix4fv(m_funcs->glGetUniformLocation(m_ID, name), 1, GL_FALSE, mat.data());
    }
